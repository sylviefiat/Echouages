...
RUN touch enable-cors.sh 
ADD enable-cors.sh $GEOSERVER_HOME/webapps/geoserver/WEB-INF

# set jetty servlets lib to enable CorsFilter
RUN wget http://central.maven.org/maven2/org/eclipse/jetty/jetty-servlets/9.4.9.v20180320/jetty-servlets-9.4.9.v20180320-config.jar -P $GEOSERVER_HOME/webapps/geoserver/WEB-INF/lib/ && \
