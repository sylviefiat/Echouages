...
class forme_HTML
{
	...
	{
		$fid = (int) $fid;
		$return_url = ($form->return_url == '') ? JRoute::_("index.php?option=com_forme&Itemid=$Itemid&fid=$fid", false) : $form->return_url;
	...
	function showForm(&$form, &$fields)
	{
		global  $Itemid, $mainframe, $params, $hide_js, $formeConfig;
		...
		$action = JRoute::_('index.php?option=com_forme&fid='.$form->id.'&Itemid='.$Itemid);
	...
	function parseFields($row){
		...
		switch($row->inputtype){
			case 'text':
				...
				$row->default_value = forme_HTML::escape($row->default_value);
				$row->name 			= forme_HTML::escape($row->name);
			...
			case 'radio':				
				...
				foreach ($row->default_value as $i=>$radioset){
					...
					$name 		 = forme_HTML::escape($row->name);
					
					$radios .= '<span class="radio'.$name.'" style="white-space: nowrap"><input type="radio" name="form['.$name.']" id="'. $name .$i. '" value="'.$valTitle[0].'" '.$row->params.' /><label for="'. $name .$i. '">'. $valTitle[1].'</label></span>';
				}
				$html = str_replace('{fieldtitle}', $row->title, $row->fieldstyle);
				$html = str_replace('{validationsign}', ($row->validation_rule) ? ' *':'', $html);
			...
			case 'checkbox':				
				...
				foreach ($row->default_value as $i=>$checkset){
					...
					$checks .= '<span class="check'.$name.'" style="white-space: nowrap"><input type="checkbox" name="form['.$name.'][]" id="'. $name .$i. '" value="'.$valTitle[0].'" '.$row->params.' /><label for="'. $name .$i. '">'. $valTitle[1].'</label></span>';
				}
				$html = str_replace('{fieldtitle}',$row->title,$row->fieldstyle);
				$html = str_replace('{validationsign}',($row->validation_rule) ? ' *':'',$html);
			...
			case 'select':
				...
				foreach ($row->default_value as $optionset){
					...
					$valTitle[1] = forme_HTML::escape($valTitle[1]);
			...
			case 'calendar':
				...
				$row->name 	   = forme_HTML::escape($row->name);
				$default_value = forme_HTML::escape($default_value);
