...
/**
 * Stranding_forms model.
 */
class Stranding_formsModelStranding_adminForm extends JModelForm
{
	...
	{
		$app = JFactory::getApplication('com_stranding_forms');

		// Load state from the request userState on edit or from the passed variable on default
        if (JFactory::getApplication()->input->get('layout') == 'edit') {
            $id = JFactory::getApplication()->getUserState('com_stranding_forms.edit.stranding_admin.id');
        } else {
	...
	{
		// Get the form.
		$form = $this->loadForm('com_stranding_forms.stranding_admin', 'stranding_adminform', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	{
		$id = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('stranding_admin.id');
        ...
        if($id) {
            ...
            $authorised = $user->authorise('core.create', 'com_stranding_forms');
            if($user->authorise('core.edit.state', 'com_stranding_forms') !== true && $state == 1){ //The user cannot edit the state of the item.
                $data['state'] = 0;
            }
        }
    ...
    {
        $id = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('stranding_admin.id');
        if(JFactory::getUser()->authorise('core.delete', 'com_stranding_forms') !== true){
            JError::raiseError(403, JText::_('JERROR_ALERTNOAUTHOR'));
            return false;
        }
        $table = $this->getTable();
