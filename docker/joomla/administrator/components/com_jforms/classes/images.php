...
class JformsImages  extends JformsCkClassFile
{
	...
	function searchFile(&$create)
	{
		...
		$fallback = $this->dir .DS. JFORMS_IMAGES_FALLBACK_NAME;
		...
		$fallback = JFORMS_IMAGES_FALLBACK_ROOT .DS. JFORMS_IMAGES_FALLBACK_NAME;
		if ($fallback && JFile::exists($fallback))
		{
			$create = 'fly';
			return $fallback;
		...
		switch($create)
		{
			case 'thumb':
				...
				else
				{
					...
					if (($size == "0x0") || in_array($size, explode(",", JFORMS_IMAGES_ALOWED_SIZES)))
					{
						$image = $this->load();
...
*/
    ...
    function writeThumb($image, $name=null)
    {
    	...
    	if (!JFORMS_IMAGES_PHYSICAL_THUMB)
	...
	{
		$width = min($width, JFORMS_IMAGES_MAX_WIDTH);
		$width = max($width, JFORMS_IMAGES_MIN_WIDTH);
		$this->width = $width;
	}

	function height($height='') {
		$height = min($height, JFORMS_IMAGES_MAX_HEIGHT);
		$height = max($height, JFORMS_IMAGES_MIN_HEIGHT);
		$this->height = $height;
	}
