...
<?php $fieldSet = $this->form->getFieldset('form.extra');?>
	<?php
	// Main folder
	$field = $fieldSet['jform_layouts'];
	$field->jdomOptions = array_merge($field->jdomOptions,array(
		'enumList' => $this->lists['select']['layouts']
	));
	?>
...
if($fake_form instanceof JForm){
	...
	foreach($fake_fieldSets as $k => $fset){
		...
		$group = array();
		$group['text'] = (!empty($fset->label)) ? $fset->label : $fset->name;
...
jQuery(document).ready(function(){	
	...
	var tmplForm = jQuery('#tmpl_'+ window['layouts']['tmpl_form'] +'_form').html();
	tmplForm = jQuery(tmplForm).wrap('<div>').parent();
	
	jQuery.each(fields,function(i,v){
		var label = tmplForm.find('#jform_layouts_\\{\\{\\=it\\.id\\}\\}_'+v+'_label'),		
		varsSelector = jQuery('#varsSelectorLayouts').clone();
		if(v != 'template'){
			varsSelector.find('option.allVariables').attr('disabled',true);			
		}
		varsSelector.find('.addVar').attr('data-target','#jform_layouts_{{=it.id}}_'+v);
	...
	jQuery('#tmpl_'+ window['layouts']['tmpl_form'] +'_form').html(tmplForm.html());
});
</script>